  ...
  return (
    <a href={articles[0]?.url} target="_blank" className="news__hero">
        ...
        <div className="news__hero-content">
            <div>
                <h3 className="card-title">{articles[0]?.title?.length > 80 ? articles[0]?.title.slice(0,80).concat("...") : articles[0]?.title}</h3>
            ...
            <span className="card-time">{new Date(articles[0]?.publishedAt).toLocaleString()}</span>
        </div>
    </a>
  )
}
